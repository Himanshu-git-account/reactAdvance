...
export const Body = () => {
    ...
    const fetchData=async ()=>{
        try{
            const data = await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=12.853532&lng=77.663033&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING");4
    ...
    return ( resList.length===0?<ShimmerCard />:
        <div className='bodyContainer'>
            <div className="filter">
                <div className="search-container">
                    ...
                    <button className="filter-button" onClick={()=>{
                        ...
                        const filteredList= resList.filter((res)=>{return res.info.name.toLowerCase().includes(searchText.toLowerCase())})
            ...
            <div className='resContainer'>
                {resList.map((restaurant) => <RestaurantDetails key={restaurant.info.id} restaurantObj={restaurant} />)}
            </div>
        </div>
    )
}
